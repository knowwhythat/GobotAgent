...
class CustomAgent(Agent):
    ...
    def __init__(
        self,
        websocket_conn: WebSocketServerProtocol,
        task: str,
        llm: BaseChatModel,
        add_infos: str = "",
    ...
    async def _log_response(self, response: CustomAgentOutput) -> None:
        ...
        await self.websocket_conn.send(
            json.dumps(
                {"done": False, "result": response.current_state.completed_contents}
            )
        )
    ...
    async def step(self, step_info: Optional[CustomAgentStepInfo] = None) -> None:
        ...
        try:
            ...
            if len(result) > 0 and result[-1].is_done:
                ...
                await self.websocket_conn.send(
                    json.dumps({"done": True, "result": result[-1].extracted_content})
                )
